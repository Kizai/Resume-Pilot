...
import time
import threading
from src.toolbox import get_conf
...
def predict_tgui(inputs, chatbot=None, history=None):
    """
        发送至chatGPT，流式获取输出。
        用于基础的对话功能。
        inputs 是本次问询的输入
    ...
    def run_coorotine(mutables):
        async def get_result(mutablec):
            async for response in run(prompt):
                print(response[len(mutablec[0]):])
                mutablec[0] = response
                if (time.time() - mutablec[1]) > 3:
                    print('exit when no listener')
                    break

        asyncio.run(get_result(mutables))
...
def predict_tgui_no_ui(inputs, top_p, temperature, history=None, sys_prompt=""):
    if history is None:
        history = []
    raw_input = "What I would like to say is the following: " + inputs
    prompt = inputs
