from src.toolbox import CatchException, report_execption, write_results_to_file, predict_no_ui_but_counting_down
import re
import unicodedata
...
def parser_resumes(file_manifest, project_folder, top_p, temperature, chatbot, history, systemPromptTxt):
    ...
    i_say = f'根据以上你自己的分析，对全文的简历内容进行概括{all_file}）。'
...
# 批量解析简历文件
@CatchException
def batch_parser_resumes(txt, top_p, temperature, chatbot, history, systemPromptTxt, WEB_PORT):
    import glob, os
    yield chatbot, history, '正常'
